...
class NeuralSnakeGame(SnakeGame):
    ...
    def handle_input(self):
        ...
        if self.input_queue:
            ...
            if last_scheduled_dir is None: last_scheduled_dir = curses.KEY_RIGHT
        ...
        for key in keys:
            ...
            valid_move = False
            if key == curses.KEY_UP and last_scheduled_dir != curses.KEY_DOWN and last_scheduled_dir != curses.KEY_UP:
                valid_move = True
                self.action_history.append(('USER', 'U'))
            elif key == curses.KEY_DOWN and last_scheduled_dir != curses.KEY_UP and last_scheduled_dir != curses.KEY_DOWN:
                valid_move = True
                self.action_history.append(('USER', 'D'))
            elif key == curses.KEY_LEFT and last_scheduled_dir != curses.KEY_RIGHT and last_scheduled_dir != curses.KEY_LEFT:
                valid_move = True
                self.action_history.append(('USER', 'L'))
            elif key == curses.KEY_RIGHT and last_scheduled_dir != curses.KEY_LEFT and last_scheduled_dir != curses.KEY_RIGHT:
                valid_move = True
    ...
    def update(self):
        ...
        # If we have a previous board and generated, include them in the prompt
        ...
        # TODO:  These functions could be defined once and reused
        # 2. Inference
        ...
        try:
             # Generate tokens
             output_ids = self.d_model.generate(context, max_new_tokens=276, stop_token_id=self.stop_token_id)
             ...
             if 'X' in generated and len(generated) < 10: # X usually stands alone or with newlines
                 self.game_over = True
             else:
                 # Parse grid
             ...
             # --- Recording ---
             # We want to record exactly what happened (input -> model -> output)
             # Reconstruct the full sequence
             # Note: generated was stripped of '$'
        ...
        for r, line in enumerate(lines):
            ...
            for c, char in enumerate(line):
                ...
                if char == 'H':
                    ...
                    body.append((r, c)) # Keep as list of tuples first, but we need set for lookup
                elif char == 'F':
                    new_food = [r, c]
