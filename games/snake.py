...
class SnakeGame:
    ...
    def draw_action_log(self, offset_y=None, offset_x=None):
        # Draw below the game board, centered
        ...
        try:
            self.stdscr.addstr(
                log_y, log_x, "Recent:", curses.color_pair(COLOR_TEXT) | curses.A_DIM
            )
        except curses.error:
            pass
        ...
        x_pos = log_x + 8
        for source, action in recent_actions:
            color = (
            ...
            try:
                self.stdscr.addstr(log_y, x_pos, action, color | curses.A_BOLD)
                x_pos += 3
            except curses.error:
                pass
