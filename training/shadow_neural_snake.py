...
from game.shared import prompt_model_selection, load_model


class ShadowNeuralSnakeGame(NeuralSnakeGame):
    """
    A version of the the game that:
    1. Validates Neural Snake's hallucinated moves against real physics.
    2. Corrects the model online if it hallucinates.
    ...
    def predict_and_verify(self, prompt, expected_board_str):
        """
    ...
    def update(self, retry=False):
        ...
        try:
            result = self.predict_and_verify(prompt, expected_board_str)
    ...
    def run_online_training(self):
        ...
        while self.divergence_detected and attempts < 10:
            ...
            # 2. Determine Weight. 50x if either is H (head snake), T (tail snake), F (food), or X (game over)
            # Why? We want to heavily penalize the model for hallucinating H (head snake), T (tail snake), F (food), or X (game over)
            weight = (
                50.0
                if any(
